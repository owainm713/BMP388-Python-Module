#!/usr/bin/env python3
"""BMP388example7, example for using the BMP388 python
module. Example using the sensor FIFO watermark interrupt and
the sensor interrupt pin to read the pressure and temperature
from FIFO.
...
# sensor FIFO watermark and interrupt pin set up and FIFO reading example
# using a Pi GPIO  and a call back function to monitor the sensor interrupt pin
...
def callback_funct(pin):
    
    # read FIFO        
    tempResults, pressResults = bmp388.read_fifo(128)        
...
GPIO.add_event_detect(interruptPin,GPIO.RISING, callback = callback_funct)
...
bmp388.set_fifo_mode(mode = 'on', fifoStopOnFull = 0, fifo_p = 1, fifo_t = 1, fifo_time = 0)
...
try:
    while True:
        # this where other code would be placed while waiting for new temperature
        # and pressure data to be available    
        time.sleep(1)
except:
